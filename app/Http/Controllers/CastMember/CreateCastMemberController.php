<?php

declare(strict_types=1);

namespace App\Http\Controllers\CastMember;

use App\Core\Application\Interfaces\Adapter\CastMember\CreateCastMemberDataAdapterInterface;
use App\Core\Application\Interfaces\Usecase\CastMember\CreateCastMemberUsecaseInterface;
use App\Core\Exception\CreateCastMemberException;
use App\Http\Controllers\Controller;
use App\Http\Helper\ResponseJsend;
use App\Http\Requests\CastMember\CreateCastMemberRequest;
use Illuminate\Http\JsonResponse;

class CreateCastMemberController extends Controller
{
    public function __construct(
        private readonly CreateCastMemberUsecaseInterface $usecase,
        private readonly CreateCastMemberDataAdapterInterface $adapter,
    ) {}

    public function __invoke(CreateCastMemberRequest $request): JsonResponse
    {
        try {
            $input = $this->adapter->fromArray($request->validated());

            $result = $this->usecase->__invoke($input);

            $response = new ResponseJsend($this->adapter->toArray($result));

            return response()
                ->json($response->toArray(), 201);
        } catch (CreateCastMemberException $e) {
            $response = new ResponseJsend(
                status: 'error',
                message: $e->getMessage(),
                code: $e->getCode(),
            );

            return response()
                ->json($response->toArray(), 400);
        } catch (\Throwable $e) {
            $response = new ResponseJsend(
                status: 'error',
                message: 'An unexpected error occurred',
                code: 500,
            );

            return response()
                ->json($response->toArray(), 500);
        }
    }
}
